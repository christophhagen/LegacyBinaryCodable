...
/**
 ...
 let decoder = LegacyBinaryStreamDecoder<Int>()
 ...
 */
final class BinaryStreamEncoder<Element> where Element: Encodable {
     ...
     */
    init(encoder: BinaryEncoder = .init()) {
    ...
    /**
     ...
     Pass the encoded data back into an instance of ``LegacyBinaryStreamDecoder`` to convert each chunk back to an element.

     - Note: Data encoded by this function can only be decoded by an appropriate ``LegacyBinaryStreamDecoder``.
     ...
     */
    func encode(_ element: Element) throws -> Data {
    ...
    /**
     ...
     - Throws: Errors of type ``EncodingError``
     */
    func encode<S>(contentsOf sequence: S) throws -> Data where S: Sequence, S.Element == Element {
