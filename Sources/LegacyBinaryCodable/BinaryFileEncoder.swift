 ...
 */
final class BinaryFileEncoder<Element> where Element: Encodable {
     ...
     */
    init(fileAt url: URL, encoder: BinaryEncoder = .init()) throws {
        if !url.exists {
    ...
    /**
     ...
     - Throws: Errors of type ``EncodingError``
     */
    func write<S>(contentsOf sequence: S) throws where S: Sequence, S.Element == Element {
