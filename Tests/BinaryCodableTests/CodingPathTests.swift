...
final class CodingPathTests: XCTestCase {
    ...
    func testStructWithCorruptArrayElement() throws {
        ...
        do {
            let _ = try LegacyBinaryDecoder().decode(KeyedBox<Test>.self, from: data)
    ...
    func testStructWithArrayMissingLastElement() throws {
        ...
        do {
            let _ = try LegacyBinaryDecoder().decode(KeyedBox<Test>.self, from: Data(corruptedBytes))
            XCTFail("Unexpected succeded!")
        } catch let error as DecodingError {
            guard case .dataCorrupted(let context) = error else {
                XCTFail("Unexpected error!")
    ...
    func testStructWithCorruptDataOnUnkeyedNestedContainer() throws {
        ...
        do {
            let _ = try LegacyBinaryDecoder().decode(KeyedBox<TestWrapper>.self, from: data)
            XCTFail("Unexpected succeded!")
        } catch let error as DecodingError {
            guard case .dataCorrupted(let context) = error else {
                XCTFail("Unexpected error!")
