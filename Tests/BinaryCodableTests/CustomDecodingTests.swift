...
final class CustomDecodingTests: XCTestCase {
    ...
    func testCustomDecoding() throws {
        ...
        let decoded = try LegacyBinaryDecoder().decode(Timestamped<String>.self, from: encoded)
    ...
    func testCustomVersionDecoding() throws {
        ...
        let encoded = try BinaryEncoder().encode(value)
        let decoded = try LegacyBinaryDecoder().decode(Timestamped<Version>.self, from: encoded)
    ...
    func testDecodingAsDifferentType() throws {
        ...
        let decoded = try LegacyBinaryDecoder().decode(String.self, from: encoded)
        let encoded2 = try BinaryEncoder().encode("1.2.3")
        print(Array(encoded2))
        let decoded2 = try LegacyBinaryDecoder().decode(Version.self, from: encoded2)
